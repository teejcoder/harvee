...
### Terminal states
`client.archived`, `project.archived`, `task.archived` are terminal for *active use* but remain readable for history and past invoices.
...
- **newState** — the intended target. On rejection, this is the *attempted* target and `accepted: false`.
